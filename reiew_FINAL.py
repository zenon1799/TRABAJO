...
def mostrar_review_guardada():
    ...
    if historial_reviews:
        if 'review_popup' in globals() and review_popup.winfo_exists():
        ...
        messagebox.showinfo("No Reviews", "Aún no hay reviews guardadas.")

def guardar_review():
    destino = destino_combobox.get()
    caracteristicas = comentario_text.get("1.0", "end").strip()
    calificacion = calificacion_var.get()
    animo = animo_var.get()
    comentario = comentario_visitante_text.get("1.0", "end").strip()

    if destino:
        review = Review(destino, caracteristicas, calificacion, animo, comentario)
        ...
        messagebox.showwarning("Destino no seleccionado", "Por favor, selecciona un destino antes de guardar la review.")
